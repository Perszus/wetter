...
class OpenMeteoProviderTest {
    ...
    fun `a rejected request becomes a provider error, not an exception`() = runTest {
        val provider = OpenMeteoProvider(clientFailing(HttpStatusCode.ServiceUnavailable), clock = clock)
    ...
    fun `an unresolvable host is reported as being offline`() = runTest {
        ...
        assertEquals(WeatherError.Offline, provider.getForecast(riga).exceptionOrNull()!!.asWeatherError())
    }
