...
object OpenMeteoMapper {
    ...
    fun toForecast(
        ...
        val zone = response.timezone?.let { runCatching { ZoneId.of(it) }.getOrNull() } ?: location.zone
    ...
    private fun toCurrent(
        ...
        return CurrentWeather(
            ...
            temperature = current?.temperature ?: fallback?.temperature ?: 0.0,
    ...
    private fun toHourly(hourly: OpenMeteoHourly, zone: ZoneId): List<HourlyWeather> =
        hourly.time.mapIndexedNotNull { index, stamp ->
            ...
            HourlyWeather(
                ...
                // and dropping it would put a hole in the timeline.
                temperature = hourly.temperature.at(index) ?: 0.0,
