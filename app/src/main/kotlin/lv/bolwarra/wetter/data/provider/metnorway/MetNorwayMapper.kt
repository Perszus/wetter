...
internal object MetNorwayMapper {
    ...
    private fun toCurrent(
        ...
        return CurrentWeather(
            observedAt = at,
            temperature = details?.airTemperature ?: hourly.firstOrNull()?.temperature ?: 0.0,
    ...
    private fun dominantCondition(steps: List<ParsedStep>, zone: ZoneId): WeatherCondition {
        val wettest = steps
            .filter { (it.precipitationContribution() ?: 0.0) >= PrecipitationIntensity.TRACE_MM_PER_HOUR }
    ...
    private class ParsedStep(val at: Instant, val step: MetNorwayTimeStep) {
        ...
        fun precipitationContribution(): Double? =
            finestPeriod()?.details?.precipitationAmount
    }
}
